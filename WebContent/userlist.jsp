...
<div class="page-content">
	                        ...
	                        <td>
		                        <select class="form-control" onchange="assignRole(this.value,'${user.name}')">
		                        	<c:forEach var="role" items="${allRoles}">
		                        		<option value="${role.id}" <c:if test="${role.name==user.rolename}">selected</c:if>>
		                        		${role.name}
		                        		</option>
		                        	</c:forEach>
		                        </select>
	...
	<div class="modal fade" id="createUserModal" tabindex="-1" role="dialog"
	  ...
	  <form id="permissionForm" name="permissionForm" action="saveUser" method="post">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-success" data-dismiss="modal"
						aria-hidden="true" onclick="return save()">保存</button> 
	...
	<script type="text/javascript">
		...
		function getNextManager(levelNo) {
		 ...
		 function save(){
			...
			/* javascript:document.getElementById('permissionForm').submit() */
			 if(document.permissionForm.name.value==""||document.permissionForm.name.value==null) 
			    { 
			      window.alert("账号不能为空！！！"); 
			      return false;
			      }
			 if(document.permissionForm.password.value==""||document.permissionForm.password.value==null) 
				{ 
				  window.alert("密码不能为空！！！"); 
				  return false;
				}
			 if(document.permissionForm.email.value==""||document.permissionForm.email.value==null) 
				{ 
				  window.alert("邮箱不能为空！！！"); 
				  return false;
				}
			  ...
			  /* if(document.getElementById('permissionForm').value==""||document.getElementById('permissionForm').value==null) 
				{ 
				      window.alert("表格信息不能为空！！！"); 
				      return false;
			    } */
